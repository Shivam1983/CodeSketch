...
import { CODE_EXECUTION_URL } from "@/config";
...
export const MonacoEditor: React.FC<MonacoEditorProps> = ({
  ...
  const runCode = async () => {
    ...
    try {
      ...
      const response = await fetch(CODE_EXECUTION_URL, {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401 || response.status === 403) {
          setOutput(
            "Error: Code execution backend is not authorized. Set PISTON_API_TOKEN in backend config.env.",
          );
          return;
        }
      ...
      if (data?.run) {
        const compileOutput = data.compile?.output
          ...
          : "";
        const runOutput =
          data.run.output || `${data.run.stdout || ""}${data.run.stderr || ""}`;
        const combinedOutput = `${compileOutput}${runOutput}`.trim();
        setOutput(combinedOutput || "Execution finished with no output.");
        return;
      }

      const message = data?.message || data?.error || "Unable to execute code.";
      setOutput(`Error: ${message}`);
    } catch (error) {
      ...
      <div className="bg-[#1e1e1e] p-2 flex justify-between items-center">
        <div className="flex items-center space-x-4">
          ...
          <select
            ...
            onChange={(e) =>
              onLanguageChange(
                e.target.value as "javascript" | "python" | "cpp",
              )
            }
            className="bg-[#2d2d2d] text-white text-sm px-2 py-1 rounded border border-[#3d3d3d] focus:outline-none focus:border-[#007acc] cursor-pointer">
            {languages.map((lang) => (
        ...
        <button
          onClick={runCode}
          className="px-4 py-1.5 bg-[#007acc] text-white rounded hover:bg-[#005999] transition-colors">
          Run Code
        </button>
      </div>
