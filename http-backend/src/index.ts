...
app.post("/signup", async (req, res) => {
  ...
  const normalizedEmail = parsedData.data.username.trim().toLowerCase();
  const normalizedName = parsedData.data.name.trim();
  ...
  try {
    const response = await fetch(PISTON_API_URL, {
      ...
      headers: {
        ...
        ...(PISTON_API_TOKEN
          ? {
              Authorization: `Bearer ${PISTON_API_TOKEN}`,
              "X-API-Key": PISTON_API_TOKEN,
            }
          : {}),
      },
    ...
    const data = await response
      .json()
      .catch(() => ({
        message: "Invalid response from code execution provider",
      }));
    ...
    res
      .status(502)
      .json({ message: "Failed to reach code execution provider" });
  }
});
